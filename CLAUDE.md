...
2. **Firebase SDKs are split by runtime.** The browser `firebase` SDK is imported only in `packages/adapters/firebase-sync`. Cloud Functions talk to Firestore directly via `firebase-admin`/`firebase-functions` — the Admin SDK belongs in `apps/cloud-functions` (~25 files) and is **not** a violation there. What the lint/depcruise rules actually forbid is **any** Firebase SDK (`firebase` or `firebase-admin`) in `domain`, `observability`, and `ui-components`. `firebase-sync` is browser-only and must never be imported by `cloud-functions` (see §8).
...
11. **PostHog SDK only in `observability`.** `posthog-js` and `posthog-node` may be imported only in `packages/adapters/observability`, which wraps them behind the `ErrorReporting`/`MatchLogging` ports. Every other package and both apps depend on the `@salt/observability` ports (default subpath in `web-pwa`, `/server` in `cloud-functions`) — never the SDK directly. Enforced by `no-restricted-imports` and depcruise's `no-posthog-outside-observability`.
...
- **Production data back-compat.** Canon, Aisles, Equipment, Shopping List, Meal Planner and Recipes hold real production data — a schema change must be backward-compatible on read, or carry a one-off migration. Recipes included: they lost greenfield status when the module shipped to all members (#240, 2026-06-17).
...
- **Wrap every AI call in `withAiTimeout`** — `withAiStreamTimeout` for `ai.generateStream` — in the file that calls the model, and declare that function's AI secrets. Bare Genkit calls have no timeout and hang the function for its full 60 s quota. `apps/cloud-functions/tests/aiTimeoutGuard.test.ts` enforces this by scanning `apps/cloud-functions/src` (issue #915).
...
projects, `.emulator-data`, the `:5174` Vite) and would kill Daniel's dev session or
corrupt another agent's e2e run: `dev`, `dev:genkit`,
...
then re-run with `SALT_TAKE_HOST=1`. You do not have to remember which:
[scripts/host-guard.mjs](scripts/host-guard.mjs) refuses them, kills nothing, and
prints what it protected, the override, the safe alternatives and the reasoning.

**Everything short of e2e runs freely anywhere, no permission needed:** `lint`,
`typecheck`, `check`, `test`, `depcruise`, `boundary:test`, `format`, `format:check`,
`pnpm -r build` (there is no root `build` script). That is how you validate work in a
worktree.
...
and unit tests need no configuration; anything needing Gemini goes through the
`FUNCTIONS_AI_FAKE` seam or does not run. Do not go hunting for keys you will never
find.
...
Rules 1–11 are machine-checked, each gate catching what the others cannot: `pnpm lint`
(eslint-plugin-boundaries, `.svelte` `<script>` blocks parsed too, so Rule 7 is
covered), `pnpm typecheck` (project references), `pnpm check` (svelte-check, template
errors `tsc` misses), `pnpm depcruise` (cycles + the resolved-path subpath rules ESLint
cannot see), `pnpm boundary:test` (violation fixtures must each error). Husky +
lint-staged block the commit; CI blocks the PR.
...
doc holds decisions, gotchas, external setup and contracts that are **not** recoverable
from the code. A doc missing from the map is invisible to every agent, so a new doc gets
its row in the same commit; `pnpm docsmap:check` and the PR doc review both route off it.
A doc earns its place by holding what code cannot say — never by restating a header
comment.

## Code search (Serena MCP)

Serena is configured **TypeScript-only** (`languages: [typescript]`) and cannot see
`.svelte` files, so a reference query for a symbol consumed only by components
returns a confident, wrong **zero**. Use it for the pure-TS layers (`domain`,
`shared-types`, the adapters, `cloud-functions`, `apps/web-pwa/src/lib/*Service.ts`);
use `grep` over `**/*.svelte` for anything the UI touches. Nor is it the impact gate —
`pnpm depcruise` / `lint` / `typecheck` encode what is _allowed_, not what is
_connected_. `.serena/` is gitignored; if it is ever regenerated, re-apply
`languages: [typescript]` and `ignored_paths: [".claude/**"]`.
