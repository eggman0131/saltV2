...
| [docs/issue-board.md](docs/issue-board.md)                                                             | `scripts/board.mjs`, `scripts/check-spec-shape.mjs`, `scripts/lib/specIssueShape.mjs`, `.github/workflows/board-status.yml`, `.github/workflows/board-dispatch.yml`, `.github/workflows/spec-shape.yml`, `.claude/commands/defect.md`, `.claude/commands/spec.md`, `.claude/commands/refactor-spec.md`, `.claude/commands/run.md` | The issue board (org project #1): what `Queue`, `Class` and `Recommended` mean, the blocker-promotion invariant and its check, why sequence is drag position and no view may carry a sort, which `Status` moves are automated, which labels were retired, and what `specced` asserts. Read before triaging, or before touching anything that writes a project field. |
...
| [docs/unit-test-spec.md](docs/unit-test-spec.md)                                                       | `packages/*/tests/**`, `packages/adapters/*/tests/**`, `apps/*/tests/**`, `**/*.test.{ts,mjs}`                                                                        | **Writing or reviewing** a Vitest unit test — non-functional rules (`UT-*`) and the reviewer checklist. The e2e spec's counterpart for the other 96k lines.                                                                                            |
...
| [docs/runbooks/recipe-times-backfill.md](docs/runbooks/recipe-times-backfill.md)                     | `scripts/backfill-recipe-times.mjs`, `apps/cloud-functions/src/flows/estimateRecipeTimes.ts`                                                                    | Re-estimating the library's prep/cook/total times (#952 phase 2) — the **deploy → dry-run → apply → verify** order per environment, and the two ways a run silently does nothing (functions not deployed; the recipe-generation kill-switch off). Read before running the backfill. |
