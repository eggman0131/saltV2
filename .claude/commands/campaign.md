...
- **The merge gate reads the whole line, so cleanup may ride with the merge.** `~/.claude/hooks/gh-merge-guard.mjs` splits a merge command into segments, verifies each PR against the queue-eligibility rule below, and clears the rest only if every segment is inspection-only or is the worktree-removal / branch-deletion for a branch that same line is merging. So the **Merge queue** recipe's steps 1 and 2 may be one chained call. What it still stops, correctly: `--admin`, a command substitution, an output redirect, anything piped to another shell, a `git branch -D` for a branch the line is not merging, and any command it does not recognise. Do not try to route around a stop — a stop is the gate telling you the line does something it could not read.
