...
Neither of those lines is somebody else's job later. Adding an issue to the project leaves **every field empty**, `Status` included — it does not land in a pile marked Triage, and an item with no `Queue` appears in no queue view, so it is simply invisible until someone happens to scroll the unfiltered board. `board.mjs check` fails on one now, which is how you find out you skipped this.
...
| **adjudicated blocking finding**  | `Defect`                                  | `Medium`, or `Recommended` per the rule below — else the floor                 | `S`                         | the ledger          |
| **follow-ups checklist** (Finish) | `Refactor`, or `Defect` if most lines are | `Low`, or `Medium` if a line has real user-facing consequence — else the floor | `S` up to 3 lines, else `M` | the ledger          |
...
**Cannot judge the band? Say so properly rather than inventing one.** The floor is `--class`, `--size` and `--status Triage`, with `Queue` left unset and the issue itself saying what evidence would settle it. A size is a property of the work and is always knowable; `Triage` is the workflow state meaning a human owes this a decision. `board.mjs check` prints an item sitting there as a note rather than a failure — it fails one claiming `Triage` without a size, and it fails an issue left with no band and no `Triage` either, because that one was not parked, it was abandoned. Filing at the end of an unattended run, expect to reach for the floor: you are the actor least able to judge a band, and the one with nobody there to correct it.

**Anything you file out of a line in another issue's list closes that line.** A previous campaign's follow-ups checklist, an epic's body list, any `- [ ]` — once its work is tracked somewhere else, the list is only useful if it says so. The `parent --of` above already gives that issue a GitHub sub-issue counter which ticks itself when the child closes; tick the line in place as well, appending the number (`- [x] <the finding, word for word> → #<new>`), with one `gh issue edit --body-file` on a body you are already holding. **Never go looking for the list** — the pointer comes from the work in hand, never a sweep over open issues hunting for checkboxes, and a list you have not read is not yours to edit.
