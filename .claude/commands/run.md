...
- **Run the whole mechanical set concurrently, in one message.** This is exactly what CI blocks on, minus the two heavy suites:
  ...
  The four beyond the obvious six are there because they are the ones a phase trips *without noticing*: a new file under `docs/` fails `docsmap:check` unless `docs-map.md` gained a row, any `packages/ui-components` edit can fail `theme:check` or `provenance:check`, and an `eslint.config.*` or `.dependency-cruiser.*` change fails `boundary:test`.
