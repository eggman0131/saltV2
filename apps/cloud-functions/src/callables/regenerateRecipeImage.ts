...
// Manual regenerate escape hatch for the recipe hero (issue #148, Tier-2),
// mirroring regenerateCanonIcon. Clearing `image` (→ null) and stamping the
// `imageRequestedAt` nonce re-fires onRecipeWritten, whose image branch then
...
// the update always mutates the doc. `imageHidden` is deleted here purely to tidy
// legacy data off the doc — the field is retired and inert, with no reader on
// either side, so there is no hidden state left to clear (see RecipeSchema).
//
...
export const regenerateRecipeImage = onCall(
  ...
  async (request) => {
    ...
    // Clear the image (→ trigger regenerates) and carry the caller's art direction.
    // imageRequestedAt forces the write to mutate the doc so the trigger fires even
    // when the recipe had no image (image already null) — see the header.
    //
