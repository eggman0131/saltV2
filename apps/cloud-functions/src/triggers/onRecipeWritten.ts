...
// script — and rewrites `metadata.phases` and `metadata.timingSummary` and
// nothing else.
 ...
 * the answer to `metadata.phases` and `metadata.timingSummary`.
 *
...
async function maybeEstimateTimes(
  ...
  try {
    const times = await estimateRecipeTimesFlow({
      ...
      // These lines are most of the hands-on estimate.
      ingredients: recipe.ingredients.flatMap((g) => g.items.map((i) => i.rawText)),
