...
// The whole of what the app asks about timing (issues #1122, #1213), and every
// authoring path — both extractors, the librarian and the re-estimator — asks
// against THIS text. A second copy anywhere is the #785 twin, and this time it
// would leave half the library's timelines drawn to one definition of "hands-on"
// and half to another.
//
// It used to be the second half of a longer block that also defined
// prepTimeMinutes / cookTimeMinutes / totalTimeMinutes, including the clause
// "totalTimeMinutes >= prepTimeMinutes + cookTimeMinutes". That clause went with
// the fields: elapsed time is now the sum of the parts by construction, so the
// invariant it defended cannot be violated and there is nothing left to
// reconcile.
//
...
//
// Exported (issue #952, phase 2) so the estimateRecipeTimes flow asks its
// question against THIS text and not a paraphrase of it. A backfill that
// re-estimated against a second, hand-copied definition would put the library
// back where it started — half the recipes on one definition and half on
// another, which is the #785 twin in a new place.
export const TIME_RULES = `- phases: the recipe's timing as an ORDERED list of 3–6 named blocks, in the order the \
