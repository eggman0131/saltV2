...
function buildPrompt(rawText: string): string {
  return [
    ...
    `3. **Leading quantity:** if the entry begins with a number (integer or decimal) optionally followed by a unit, extract them as "amount" (JSON number) and "unit" (string). The name is what remains after the number and unit. If no number is present, or extraction would leave an empty name, omit both fields. Examples: "2kg maris piper potatoes" → amount 2, unit "kg", name "maris piper potatoes"; "3 onions" → amount 3, name "onions"; "a couple of onions" → no amount, no unit.`,
