...
function buildPrompt(rawText: string): string {
  return [
    ...
    `4. **Trailing quantity.** ONLY when rule 3 found nothing, and applied to the name AFTER the rule-1 "for" split. Only shapes 3b and 3c have a trailing form — a written cardinal (3a) never appears at the end of the line, so "onions two" gets no quantity at all, not amount 2. The two shapes that do, at the end of the line: "cucumber 400g" → amount 400, unit "g", name "cucumber"; "potatoes 1 kg" → amount 1, unit "kg", name "potatoes" (recognised unit); "onions 3" → amount 3, name "onions" (bare trailing count). A trailing word that is not a recognised unit stays in the name.`,
