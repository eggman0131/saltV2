<script lang="ts">
  ...
  //   neutral   a fact about the DOCUMENT rather than about the dish: who added
  //             the recipe.
  //
  // It carried two more tints — teal for Prep, terracotta for Cook — until #1213
  // took the three duration chips off this row and left the timeline to state a
  // recipe's timing on its own. One tint and a default is not a smaller version
  // of that scheme; it is what is left once there is only one kind of number.
  // Nothing is carried by colour ALONE in either case — every chip says its own
  // kind in words, so the tint only lets the row be scanned instead of read
  // (ui-spec-v02 §7).
  ...
  // here, and everything below reads that list — the template never asks the
  // recipe for it again. `recipePhaseTotals` then sums exactly what is drawn, and
  // is the app's single funnel for "how long does this take".
  ...
  const facts = $derived.by((): RecipeFact[] => {
    ...
    // Serves is a COOKING fact. An outing has none, and gating here covers the
    // chip and, through `hasMeta`, the card.
    if (isCookable(kindOf(recipe))) {
      ...
      // NO TIMING CHIP, deliberately (issues #1122, #1213). The Prep / Cook /
      // Total chips that stood here are gone with the fields behind them, and
      // nothing derived from the phases replaces them: the timeline a few lines
      // below states its own total, and a chip repeating it would be #1122's own
      // complaint — two accounts of one fact, side by side — at a smaller scale.
    }
    ...
    >
      ...
      <div class="flex min-w-0 flex-col gap-4 split:min-h-0 split:overflow-y-auto">
        ...
        <!-- Description, facts, tags and the phase strip.
             ...
             rather than `phases.length` — the single funnel
             docs/recipe-module.md names (issue #1122 review, should-fix 6). -->
        {#if recipe.description || facts.length > 0 || recipe.metadata.tags.length > 0 || sourceUrl || phaseTotals.hasPhases}
          <Card>
            <CardContent class="flex flex-col gap-3 p-4">
              ...
              {/if}
              <!-- The planning timeline (issue #1122), and since #1213 the ONLY
                   timing graphic on the page: the #878 cook-shape ribbon that sat
                   beneath it is deleted. That ribbon was built from whichever
                   steps happened to carry a countdown — the original defect
                   #1122 was raised against — and every recipe now has a real
                   strip, so its suppression condition was always true.
                        ...
                        <div class="flex min-w-0 flex-1 flex-col gap-2">
                          ...
                          {#if handsOff || step.timer}
                            <div class="flex flex-wrap items-center gap-1.5">
                              {#if handsOff}
                                <!-- Sage, the colour the phase timeline above already
                                     uses for hands-off time, so the two agree about
                                     what a wait looks like on this page. -->
                                <span
