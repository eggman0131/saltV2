<script lang="ts">
  ...
  const metadataCards = $derived.by(() => {
    ...
    // One "Timing" card, present when either half of the pair moved. It is the
    // whole of the recipe's timing (issue #1213): the Prep / Cook / Total cards
    // that stood beside it are gone with the fields they read, because showing a
    // Prep time beside a phase breakdown is the split definition #1122 exists to
    // end.
  ...
  // Timing card declines to draw a pair whose two rendered sides read the same
  // (#1217), and `proseCard` declines a prose field moving null ↔ "". So the two
  // can disagree, and when they do, `hasChanges` alone would draw the "here's
  // what will change" sheet with a live Apply button over zero cards (#1216).
  // Deciding from the cards actually built keeps this exact everywhere
