...
- [ ] Pushover secrets (`PUSHOVER_APP_TOKEN`, `PUSHOVER_USER_KEY`) — **not set in any environment yet** (#680). One shared family account, so the SAME pair goes to dev, staging and production; the non-prod blast radius is handled in code (only `daniel-*` devices are targeted outside production), not by separate credentials. Until they are set the Pushover sink logs "not provisioned" and no-ops, and cook timers fall back to web push alone — which is the late-delivery problem #680 exists to fix, so an unset secret looks exactly like no fix at all. Also confirm both one-time licences (Android + iOS, ~$10) are live: the 30-day trial lapsing presents identically to the bug.
