...
3. **Firestore write triggers** (`onShoppingListItemWrite`, `onCanonItemWritten`, `onRecipeWritten`, `onCookTimerWrite`, `onKitchenTimerWrite`) — respond to document writes and run domain logic server-side, writing results back to Firestore. `onCookTimerWrite` diffs newly-armed `notify` timers on a cook session write and enqueues a Cloud Task at `scheduleTime = endsAt` for accurate push delivery. `onKitchenTimerWrite` (issue #842) is a full sibling of it bound to `kitchenTimers/{uid}` rather than a branch inside it — a standalone timer has no cook session to re-read — diffing newly-armed timers the same way and enqueueing onto its own `onKitchenTimerDispatch` queue.
