...
- Entities and value objects for:
  ...
  - Cook sessions (CookSession — per-user in-progress cook state; `ownerUid`-scoped with a deterministic `${recipeId}_${uid}` id, no TTL; Phase 1 holds mise-en-place `checkedIngredientIds`, with `completedStepIds` and `activeTimers` reserved for later phases)
