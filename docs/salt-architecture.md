...
- **Exception — cook sessions:** The `cookSessions` collection is the second owner-scoped collection, following the same exception because an in-progress cook (mise-en-place ticks and, in later phases, step progress and timers) is personal, not family-shared. Firestore rules mirror `chatSessions` (`ownerUid == request.auth.uid`, set on create and pinned on update); the id is deterministic (`${recipeId}_${uid}`) so a user has exactly one resumable session per recipe. One rule detail differs from `chatSessions` and is load-bearing (issue #558): read and delete also permit `resource == null`. Because the id is deterministic the cook page subscribes to it *before* the session exists and only then bootstraps one, and a rule that dereferences `resource.data` cannot be evaluated against an absent document — Firestore denies the read and tears the listener down permanently, so the first cook of a recipe would run with a dead subscription and report a spurious `permission-denied`. `chatSessions` needs no equivalent because it is only ever read through a collection query filtered on `ownerUid`. Absence carries no data, and every session that exists stays owner-only on read, update and delete. Unlike `chatSessions` there is **no TTL** — a cook may span several days — and orphan cleanup (a session whose recipe was deleted) is handled client-side by the cook page, which deletes the session on open.
