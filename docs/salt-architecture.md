...
1. **Gen-AI callables** (`embedText`, `arbitrateCanon`, `matchOrCreateCanon`, `canonicaliseRecipeIngredients`, `parseRecipeIngredients`, `identifyEquipment`, `populateEquipmentEntry`, `regenerateCanonIcon`, `regenerateRecipeImage`, `describeRecipeScene`, `chefChat`, `authorRecipe`, `generateChatTitle`, `extractRecipeFromUrl`, `extractRecipeFromPhoto`, `generateGuidedPlan`, `extractProcessStages`, `proposeSchedule`, `drawEquipmentIcon`) — HTTPS callables invoked by the client. Every one takes its App Check setting from the shared `APP_CHECK_ENFORCEMENT` constant — see "App Check posture" below. Notable variants:
