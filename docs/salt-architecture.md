...
9. **Cloud Scheduler maintenance jobs** (`sweepOrphanedStorage`) — `onSchedule` functions that run on a fixed cron with no client caller. `sweepOrphanedStorage` runs weekly (Sunday 03:00 Europe/London) and deletes Storage objects whose Firestore document is gone: it lists each prefix (`canon-icons/`, `recipe-images/`) and does an id-only `.select()` scan of the matching collection, then removes objects whose id is absent from the live set, subject to a 7-day age grace (guards against the list/scan window and partial scan false-positives) and a 500-object per-run cap (blast-radius limit). Never throws — errors are reported via `reportServerError` (`StorageError` category) and left for the next weekly run. Idempotent; `retryCount: 0` (a retry storm on a broken run buys nothing that next Sunday would not).
