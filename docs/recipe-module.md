...
- **`parsed.unit` is metric-or-nothing** — `'g' | 'ml' | null`. `null` means the
  ...
  "½ tsp" still reads as "½ tsp" — but only up to a **3 tbsp** volume-measure cap
  (the reader-facing `READER_UNIT_PRINCIPLE`, `@salt/domain/prompts/unitPolicy.ts`,
  issue #934): above that, the spoon has stopped being the useful way to read the
  amount, so `displayText` is `null` and the amount reads metric-only.
