...
### Trace context unification (server-side, env-gated)

> **Status: server-side trace unification is env-gated.** Browser→CF trace linking (minting a fresh browser traceparent and propagating it on the wire) stays deferred — the vestigial browser→CF `_trace` payload field was **removed**, so there is no `_trace` on the callable wire.

Each CF invocation renders as one coherent trace by reading the inbound W3C trace context from the request headers, not from the payload. In production the `matchOrCreateCanon` callable extracts the inbound `traceparent` / `tracestate` from `request.rawRequest.headers` and runs the Genkit flow within that OTel context via [`runWithExtractedTraceContext`](../packages/adapters/observability/src/server/init.ts), so the `canon.matchOrCreateCanon` flow span **nests under the platform request span** instead of re-rooting a fresh trace. The single process-wide tracer provider is owned by `enableFirebaseTelemetry()`; this helper only operates on the active context and degrades to a plain call when there is no usable header or registered propagator.

**Env gate:** the callable suppresses `runWithExtractedTraceContext` when `GENKIT_TELEMETRY_SERVER` is set (local `pnpm dev:emulators`) so flows stay **root-listed** in the Genkit Dev UI; it is honoured in production for one coherent trace. This env-gate is what resolved the 2026-05-11 regression that originally parked propagation (a non-root flow span was filtered out of the Genkit Dev UI's trace list). `setActiveSpanName` is called inside the flow body to append the item name to the otherwise-generic flow span so it is scannable in the Genkit / Cloud trace view.
