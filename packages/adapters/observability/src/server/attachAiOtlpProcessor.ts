...
// Attach the PostHog AI-OTLP span processor to the process-wide OTel provider
// that Genkit owns. enableFirebaseTelemetry() → genkit core does
...
// "no AI traces shipped" rather than a crash at module load.
//
// Best-effort, never throws (CLAUDE.md Rule 10). Suppressed when:
...
export function attachAiOtlpSpanProcessor(provider: unknown = trace.getTracerProvider()): void {
  try {
    ...
    if (typeof target.addSpanProcessor === 'function') {
      target.addSpanProcessor(aiOtlpSpanProcessor);
    }
  } catch {
