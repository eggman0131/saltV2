...
// (cross-recipe collapse) — so the shopping-list outcome cannot silently regress:
//   "3 egg whites"                     → Free Range Egg ×3
//   "2 egg yolks" + "3 egg whites"     → one Free Range Egg ×3 (MAX, not 5)
//   "6 chicken thighs" (2 per bird)    → Whole Chicken ×3
// The downstream buildSubtotals bucketing (Σwhole + MAX-form in one 'count' row,
// "×3 not 350 g") is locked separately in shoppingList/groupItemsByAisle.test.ts.
...
describe('count-yield resolution — maxCountWinners (cross-recipe MAX, not sum)', () => {
  it('two recipes on one parent collapse by MAX, not sum (2 yolks + 3 whites → 3 eggs, not 5)', () => {
    ...
    const yolks = formParentCount(2, 'count', eggYolk)!; // recipe A: 2 egg yolks → 2
    const whites = formParentCount(3, 'count', eggWhite)!; // recipe B: 3 egg whites → 3
  ...
  it('the chicken ratio survives the cross-recipe MAX (6 thighs→3 vs 2 drumsticks→1 = 3 birds)', () => {
